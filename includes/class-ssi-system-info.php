...
class SSI_System_Info {
	 ...
	 */
	public static function get_all( $include_developer = false ) {
		$cache_key = $include_developer ? self::CACHE_KEY_DEV : self::CACHE_KEY;
		...
		if ( $include_developer ) {
			$data['developer'] = self::get_developer_info();
		}
	...
	public static function purge_cache() {
		...
		delete_transient( self::CACHE_KEY_DEV );
	}
	...
	/**
	 * Collect extended data for developer mode. Cached for 12 hours.
	 *
	 * @return array<string, mixed>
	 */
	public static function get_developer_info() {
		return array(
			'php_extensions'   => self::get_php_extensions(),
			'database_size_mb' => self::get_database_size_mb(),
			'top_tables'       => self::get_top_5_tables(),
			'query_count'      => (int) $GLOBALS['wpdb']->num_queries,
			'object_cache'     => wp_using_ext_object_cache()
								  ? __( 'External (Redis/Memcached)', 'server-site-insight' )
								  : __( 'WordPress default', 'server-site-insight' ),
			'rest_routes'      => self::count_rest_routes(),
		);
	}

	/**
	 * Sorted list of loaded PHP extensions.
	 *
	 * @return array<int, string>
	 */
	private static function get_php_extensions() {
		...
		sort( $ext );
		return array_map( 'sanitize_text_field', $ext );
	}

	/**
	 * Total database size in MB, cached for 12 hours to avoid heavy queries.
	 *
	 * @return float
	 */
	private static function get_database_size_mb() {
		$cached = get_transient( 'ssi_db_size' );
		if ( false !== $cached ) {
			return (float) $cached;
		}
		...
		$size = ( $rows && isset( $rows[0]->size ) ) ? round( (float) $rows[0]->size / 1024 / 1024, 2 ) : 0.0;
		set_transient( 'ssi_db_size', $size, 12 * HOUR_IN_SECONDS );
		return $size;
	}

	/**
	 * Get top 5 largest database tables. Cached for 12 hours.
	 *
	 * @return array<int, array<string, mixed>>
	 */
	private static function get_top_5_tables() {
		$cached = get_transient( 'ssi_top_5_tables_v2' );
		if ( false !== $cached ) {
			return $cached;
		}
		...
		$rows = $wpdb->get_results(
			$wpdb->prepare(
				'SELECT TABLE_NAME AS name, (DATA_LENGTH + INDEX_LENGTH) AS size FROM information_schema.TABLES WHERE TABLE_SCHEMA = %s ORDER BY (DATA_LENGTH + INDEX_LENGTH) DESC LIMIT 5',
				DB_NAME
			)
		);
		...
		if ( $rows ) {
			foreach ( $rows as $row ) {
				$tables[] = array(
					...
					'size_mb' => round( (float) $row->size / 1024 / 1024, 2 ),
				);
			}
		}

		set_transient( 'ssi_top_5_tables_v2', $tables, 12 * HOUR_IN_SECONDS );
		return $tables;
	}

	/**
	 * Count registered REST routes.
	 *
	 * @return int
	 */
	private static function count_rest_routes() {
